...
async function renderLichThi(list_lichthi) {
    ...
    try {
        ...
        var htmls = list_lichthi.map(function(lichthi, index) {
            ...
            return `
                <tr class="lichthi-item-${lichthi.id}">
                    <td>${index + 1}</td>
                    <td>${chungChi ? chungChi.ten_chung_chi : ''}</td>
                    <td>${lichthi.ngay_thi || ''}</td>
                    <td>${lichthi.gio_thi || ''}</td>
                    <td>${lichthi.sl_toi_da || ''}</td>
                    <td>${lichthi.so_luong_dk || 0}</td>
                    <td class="edit-icons">
                        <button type="button" class="btn btn-sm btn-warning" onclick="editLichThi('${lichthi.id}')"><i class="bi bi-pencil"></i></button>
                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteLichThi('${lichthi.id}', getLichThi.bind(null, renderLichThi))"><i class="bi bi-trash"></i></button>
                    </td>
                </tr>
            `;
        }).join('');
...
async function addLichThi() {
    ...
    if (tenChungChiInput && ngayThiInput && gioThiInput && slToiDaInput) {
        ...
        const trungLich = lichThiList.filter(lt =>
            lt.id_chung_chi === id_chung_chi && lt.ngay_thi === ngay_thi
        );

        const [hMoi, mMoi] = gio_thi.split(':').map(Number);
        const timeMoi = hMoi * 60 + mMoi;

        for (const lt of trungLich) {
            const [hCu, mCu] = lt.gio_thi.split(':').map(Number);
        ...
        var data = {
            id_chung_chi: id_chung_chi, // giữ nguyên là chuỗi
            ngay_thi: ngay_thi,
            gio_thi: gio_thi,
            sl_toi_da: Number(sl_toi_da),
            so_luong_dk: 0 // Mặc định khi tạo mới
        };
            ...
            if (lichThi) {
                ...
                fetchChungChi().then(function(chungChiList) {
                    ...
                    btnSave.onclick = async function() {
                        ...
                        const trungLich = lichThiList.filter(lt =>
                            lt.id_chung_chi === id_chung_chi &&
                            lt.ngay_thi === ngay_thi &&
                            lt.id !== id
                        );
                        ...
                        const timeMoi = parseTimeToMinutes(gio_thi);

                        for (const lt of trungLich) {
                            const timeCu = parseTimeToMinutes(lt.gio_thi);
                        ...
                        var formData = {
                            id_chung_chi: id_chung_chi,
                            ngay_thi: ngay_thi,
                            gio_thi: gio_thi,
                            sl_toi_da: Number(sl_toi_da),
                            so_luong_dk: lichThi.so_luong_dk || 0
                        };
    ...
    fetch(lichThiAPI)
        ...
        .then(async function(listLichThi) {
            ...
            const filtered = listLichThi.filter(function(lichthi) {
                if (searchDay && lichthi.ngay_thi !== searchDay) {
                    return false;
                }
                if (searchTerm) {
                    const chungChi = chungChiList.find(
                        c => String(c.id) === String(lichthi.id_chung_chi)
                    );
                    if (!chungChi || !chungChi.ten_chung_chi.toLowerCase().includes(searchTerm)) {
                        return false;
                    }
                }
                return true;
